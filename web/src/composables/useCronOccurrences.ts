...
import { normalizeOffsetLabel, toOrdinal } from './useCronFieldParsers';
import {
  analyzeLocalPattern,
  generateOccurrences,
...
export function renderScheduleDescription(
  ...
  if (analysis.occurrences.length === 0) {
    return 'No scheduled occurrences in the next period';
  }
...
function tryRenderSpecificDateShiftDescription(
  ...
  if (uniqueMonthDayPairs.length > 12) {
    return null;
  }
...
function validateLastWeekday(parts: string[], dow: string): boolean | null {
  const m = /^([0-7A-Za-z]+)L$/i.exec(dow);
  if (!m) return null;
