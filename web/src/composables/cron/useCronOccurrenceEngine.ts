...
function resolveDowMatch(
  ...
  for (const token of dowField.split(',').map(part => part.trim())) {
    const nthMatch = token.match(/^([0-7A-Za-z]+)#([1-5])$/i);
    if (nthMatch) {
      if (matchesNthWeekday(date, utcDay, nthMatch[1], parseInt(nthMatch[2], 10))) {
        return true;
      }
      continue;
    }

    const lastMatch = token.match(/^([0-7A-Za-z]+)L$/i);
    if (lastMatch) {
      if (matchesLastWeekday(date, utcDay, month, year, lastMatch[1])) {
        return true;
      }
      continue;
    }

    const dows = parseDayOfWeekField(token);
    if (dows.includes(utcDay)) {
      return true;
    }
  }
...
function matchesDayOfMonth(date: number, domField: string, year: number, month: number): boolean {
  return collectDayOfMonthMatches(domField, year, month).includes(date);
}
