...
@DisplayName("ArcGISIntegrationService - @PublishNotification wiring")
class ArcGISIntegrationServiceNotificationAopTest {
    ...
    class TriggerJobExecutionAnnotation {
        ...
        private PublishNotification resolveAnnotation() throws NoSuchMethodException {
            Method method = ArcGISIntegrationService.class.getMethod(
        ...
        @Test
        @DisplayName("event key is ARCGIS_INTEGRATION_JOB_ADHOC_RUN")
        void event_key_is_arcgis_job_adhoc_run() throws NoSuchMethodException {
            assertThat(resolveAnnotation().eventKey())
                    .isEqualTo(NotificationEventKey.ARCGIS_INTEGRATION_JOB_ADHOC_RUN);
        }

        @Test
        @DisplayName("metadata provider bean name is arcGISNotificationMetadataProvider")
        void metadata_provider_bean_is_arcgis_provider() throws NoSuchMethodException {
            assertThat(resolveAnnotation().metadataProvider())
                    .isEqualTo("arcGISNotificationMetadataProvider");
        }
    ...
    class TriggerJobExecutionAopBehaviour {
        ...
        void setUp() throws Throwable {
            ...
            when(applicationContext.getBean(
                    "arcGISNotificationMetadataProvider", NotificationMetadataProvider.class))
                    .thenReturn(metadataProvider);
            when(metadataProvider.resolve(INTEGRATION_ID, TENANT_ID))
                    .thenReturn(Map.of(
                            "integrationName", "My ArcGIS Integration",
                            "integrationId", INTEGRATION_ID.toString()));
        }

        @Test
        @DisplayName("publishes event with ARCGIS_INTEGRATION_JOB_ADHOC_RUN key")
        void publishes_event_with_correct_event_key() throws Throwable {
            ...
            assertThat(captor.getValue().getEventKey())
                    .isEqualTo(NotificationEventKey.ARCGIS_INTEGRATION_JOB_ADHOC_RUN.name());
        }

        @Test
        @DisplayName("arcGISNotificationMetadataProvider is called with integration ID and tenant ID")
        ...
        @Test
        @DisplayName("published event contains metadata returned by arcGISNotificationMetadataProvider")
        void published_event_contains_provider_metadata() throws Throwable {
            ...
            assertThat(captor.getValue().getMetadata())
                    .containsEntry("integrationName", "My ArcGIS Integration")
